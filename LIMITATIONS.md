...
## "Stable API" usage to note (brief asked to flag this)

We used the **Beta** API where available (`companies`, `products`, `plans`, `checkout-configurations`, `transfers`, `ledger-accounts`, `account-links`), pinned via `Api-Version-Date`. Documented **Stable-only** fallbacks: `GET /payments/{id}` (retrieve payment), **memberships**, and **webhooks**.
